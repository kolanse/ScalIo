...
object TestUsersData {
    ...
    val fakeUser = User(avatarUrl = fakeUserApi.avatarUrl ?: "", login = fakeUserApi.login ?: "", type = fakeUserApi.type ?: "")

    fun generateFakeUserApiModel(): List<UserApiItem> {

        return (0..PER_PAGE_TEST).map {
            fakeUserApi.copy(login = "$FAKE_SEARCH_QUERY$it", id = it)
        }
    }
